            ...
            <div class="feed-list">
                <div class="event-list">
                    <div v-for="event in events">
                        <PushEvent :event="event" :key="event.id" v-if="event.type==='PushEvent'"></PushEvent>
                        <PullRequestEvent :event="event" :key="event.id" v-if="event.type==='PullRequestEvent'"></PullRequestEvent>
                        <CreateEvent :event="event" :key="event.id" v-if="event.type==='CreateEvent'"></CreateEvent>
                        <WatchEvent :event="event" :key="event.id" v-if="event.type==='WatchEvent'"></WatchEvent>
                        <DeleteEvent :event="event" :key="event.id" v-if="event.type==='DeleteEvent'"></DeleteEvent>
                        <IssuesEvent :event="event" :key="event.id" v-if="event.type==='IssuesEvent'"></IssuesEvent>
                        <IssueCommentEvent :event="event" :key="event.id" v-if="event.type==='IssueCommentEvent'"></IssueCommentEvent>
                        <ForkEvent :event="event"  :key="event.id" v-if="event.type==='ForkEvent'"></ForkEvent>
                        <CommitCommentEvent :event="event" :key="event.id" v-if="event.type==='CommitCommentEvent'"></CommitCommentEvent>
                        <PublicEvent :event="event"  :key="event.id" v-if="event.type==='PublicEvent'"></PublicEvent>
                    </div>
                </div>
            </div>
        </div>
